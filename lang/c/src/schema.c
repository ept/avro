...
static char *split_namespace_name(const char *fullname, const char **name_out)
{
	int i = 0, last_dot = -1;
	while (fullname[i]) {
		if (fullname[i] == '.') last_dot = i;
		i++;
	}

	if (last_dot >= 0) {
		*name_out = &(fullname[last_dot + 1]);
		return avro_strndup(fullname, last_dot);
	} else {
		*name_out = fullname;
		return NULL;
	}
	...
	case AVRO_ENUM:
		{
			...
			if (strchr(fullname, '.')) {
				...
				*schema = avro_schema_enum(name, namespace);
				...
				const char *namespace = json_string_value(json_namespace);
				*schema = avro_schema_enum(fullname, namespace);
			} else {
				*schema = avro_schema_enum(fullname, parent_namespace);
			}
	...
	case AVRO_FIXED:
		{
			...
			if (strchr(fullname, '.')) {
				char *namespace;
				namespace = split_namespace_name(fullname, &name);
				*schema = avro_schema_fixed(name, namespace, (int64_t) size);
				avro_str_free(namespace);
			} else if (json_is_string(json_namespace)) {
				const char *namespace = json_string_value(json_namespace);
				*schema = avro_schema_fixed(fullname, namespace, (int64_t) size);
			} else {
				*schema = avro_schema_fixed(fullname, parent_namespace, (int64_t) size);
			}
